...
export function getSkyTransition(date: Date): SkyTransition {
  ...
  for (let i = 0; i < stops.length; i++) {
    ...
    if (progress >= pos && progress < boundary) {
      const segmentSpan = boundary - pos;
      const elapsedSpan = progress - pos;
      const segmentProgress = segmentSpan > 0 ? elapsedSpan / segmentSpan : 0;

      const totalDurationSec = Math.round(segmentSpan * 24 * 3600);
      const elapsedDurationSec = Math.round(elapsedSpan * 24 * 3600);
  ...
  const firstDurationSec = Math.round(stops[1][1] * 24 * 3600);
  ...
  return {
    ...
    isDaytime: true,
    segmentProgress: 0,
    totalDurationSec: firstDurationSec,
    elapsedDurationSec: 0,
    progress: 0,
    dayFraction: dayFraction || 0.5,
  };
}
